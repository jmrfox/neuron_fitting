...
# Fitted to FI curve from Goddard 2014 using scipy dual annealing
...
def run_optimization(
    ...
    **da_kwargs,
    ...
    Args:
        ...
        no_local_search: If True, perform traditional generalized simulated annealing
                        without local search. Default is False (with local search).
        **da_kwargs: Additional keyword arguments for dual_annealing.

    Returns:
        OptimizeResult from scipy.optimize.dual_annealing.
    """
    bounds = get_bounds_for_de(parameter_bank)
    n_params = len(bounds)

    print(f"Starting dual annealing optimization")
    print(f"Number of parameters: {n_params}")
    print(f"Max iterations: {maxiter}")
    print(f"Local search: {not no_local_search}")
...
if __name__ == "__main__":
    # Example usage
    print("=" * 80)
    print("AdEx-LIF Parameter Optimization using Dual Annealing")
    print("=" * 80)
    ...
    run_label = f"scipy_dualann_{neuron_type}"
